function varargout = solveWLS(prs, sat_pos, x0, options)        
    ...
    if options.verbose == 1
        ...
        hvpv = display(idx,vtPv,'v^TPv');
        hcnc = display(idx,ctNinvc,'c^TN^{-1}c');
        h1 = display(idx,ctNinvc./vtPv,'\frac{c^TN^{-1}c}{v^TPv}');
        for i = 1:p
            h(i) = display(idx,X(i,:),strcat('x(', num2str(i),') v.s. iteration'));
        end
        ...
        %% ellipsoid drawing
        metrics = {'m','m','m'};
        he1 = drawConfidenceEllipsoid(Dx, 'chi', metrics);
        he2 = drawConfidenceEllipsoid(Dx, 'F', metrics);
