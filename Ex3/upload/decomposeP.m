function [K, R, t, c] = decomposeP(P)
% function [K, R, t, c] = decomposeP(P)
%
% Decompose projection matrix to camera intrinsics, rotation matrix, camera center, and translation.
%
%   Inputs:
%       P: projection matrix.
%   Outputs:
...
%   is not liable for any damage that its utilization may cause.
    ...
    D = diag([sign(K(1,1)), sign(K(1,1)), 1]);
    K = K * D;
    R = D * R;
    t = -R*c;
    K = K./K(end,end);
