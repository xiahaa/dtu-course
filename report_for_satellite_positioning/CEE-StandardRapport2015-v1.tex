\documentclass[a4paper,11pt,dvipsnames, oneside]{memoir} %openright if a more "booklike report is wished, %,twoside,openany

\usepackage{listings}
\usepackage{color} %red, green, blue, yellow, cyan, magenta, black, white
\definecolor{mygreen}{RGB}{28,172,0} % color values Red, Green, Blue
\definecolor{mylilas}{RGB}{170,55,241}
\setlength\extrarowheight{2pt}
\usepackage{rotating}



\lstset{language=Matlab,%
	%basicstyle=\color{red},
	breaklines=true,%
	morekeywords={matlab2tikz},
	keywordstyle=\color{blue},%
	morekeywords=[2]{1}, keywordstyle=[2]{\color{black}},
	identifierstyle=\color{black},%
	stringstyle=\color{mylilas},
	commentstyle=\color{mygreen},%
	showstringspaces=false,%without this there will be a symbol in the places where there is a space
	numbers=left,%
	numberstyle={\tiny \color{black}},% size of the numbers
	numbersep=9pt, % this defines how far the numbers are from the text
	emph=[1]{for,end,break},emphstyle=[1]\color{red}, %some words to emphasise
	%emph=[2]{word1,word2}, emphstyle=[2]{style},    
}


\input{preamble}
\input{files/info}


\begin{document}


%%%%% FRONTPAGE %%%%%
\include{files/frontpage}


%%%%% FRONT MATTER %%%%%
\frontmatter 
%\newpage\thispagestyle{empty}\mbox{}	 % Creates blank page
%\include{files/preface}
\include{files/abstract} 
%\include{files/resume} 
%\include{files/symbols} 

%%%%% ToC & FIXME %%%%%
\tableofcontents


%%%%% MAIN MATTER %%%%%%
\mainmatter
%\include{files/}
%\include{files/assign_a}
%\include{files/assign_b}
%\include{files/assign_c}
%\include{files/assign_d}
%\include{files/assign_e}
%\include{files/assign_f}
%\include{files/assign_f_ext}

\include{files/assign_g}
\include{files/assign_h}

%\include{files/reportWriting}
%\include{files/layoutTemplate}
%\include{files/reportHandin}
%\include{files/conclusion}

% in documenet


%%%%% BIBLIOGRAPHY %%%%% 
\begingroup
	\raggedright
	\bibliography{bibtex/litteratur}
\endgroup


%%%%% APPENDICES %%%%%
%\newpage\thispagestyle{empty}\mbox{}
%\begin{appendices}
%	\include{appendices/A}
%	\include{appendices/B}
%\end{appendices}




\include{files/backpage}



	
	 				
\end{document}